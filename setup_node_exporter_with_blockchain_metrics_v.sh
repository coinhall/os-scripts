...
prom_script_file=vnode_status_prom_script.py
...
echo "Downloading vnode_status_prom_script.py"
if [ -f "$prom_script_file" ]; then
    rm -f $prom_script_file
fi
wget https://raw.githubusercontent.com/coinhall/os-scripts/main/vnode_status_prom_script.py -P ~
echo "$prom_script_file downloaded."

echo "Setting up cronjob..."
(crontab -l || true; echo "*/1 * * * * python3 ~/vnode_status_prom_script.py $1 > ./node_exporter_cron.log 2>&1") | awk '!seen[$0]++' | crontab -
